function searchGoogleBooks(){
    var txtSearch = document.querySelector("#txtKey");
    if(txtSearch.value == ""){
        document.querySelector("#mainDiv").classList.add("hasError");
        document.querySelector("#mainDiv").innerHTML = "You did not provide any search key words";
        return;
    }

    document.querySelector("#mainDiv").classList.remove("hasError");

    fetch('https://www.googleapis.com/books/v1/volumes?&q=' + txtSearch.value +"&startIndex=0&maxResults=27")
    ...
    .then(json => {
        var list = `<div class="row col-sm-5>found ${json.totalItems} books </div>
         ...
         for(let i = 0; i< json.items.length; i++){
            ...
            var template = `<ul class ="differentColor col-sm-12 col-md-6 col-xl-4">`;
            template += `<a class= "list-group-item differentColor list-group-item-action active" href="${json.items[i].selfLink}">${bookInfo.title}</a>` 
